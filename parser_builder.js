...
import { TokenType, Fail, jlog, isTruthy, isFalsy, } from "./util.js"
export function $and(...fns) {
    return function (slice, fn_ind) {
        let gap = 0, results = [];
        for (let i = 0; i < fns.length; i++) {
            if (gap > slice.length&&slice.done ===true) return Fail;
...
export const $kw = (value) => {
    return function (slice) {
        let $1 = slice.get(0)
        if (!$1) return Fail
        return $1[0] == TokenType.IDENTIFIER && $1[1] == value ? [true, 1, { type: TokenType.IDENTIFIER, value: $1[1] }] : Fail
    }
}
...
export const $symbols = (value) => {
    return function (slice,fn_ind) {
        for (let i = 0; i < slice.length; i++) {
            let $1 = slice.get(i)
            if (!$1) return Fail
            if (!($1[0] == TokenType.SYMBOL && $1[1] == value[i])) return Fail

            if (i == value.length - 1) return [true, i + 1, { type: TokenType.SYMBOL, value: value }]
        }
        return Fail
    }
}
...
export const $indent = _ => {
    return function (slice) {
        let $1 = slice.get(0)
        if (!$1) return Fail
        return $1[0] == TokenType.INDENT ? [true, 1, { type: TokenType.INDENT, value: $1[1] }] : Fail
    }
}
